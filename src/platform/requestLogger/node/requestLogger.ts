...
// Side channel for passing usage data across VS Code IPC boundary.
// Used by BYOK providers to report real token usage back to ExtensionContributedChatEndpoint.
const usageCorrelationMap = new Map<string, { prompt_tokens: number; completion_tokens: number; total_tokens: number; prompt_tokens_details?: { cached_tokens: number } }>();

export function storeUsageForCorrelation(correlationId: string, usage: { prompt_tokens: number; completion_tokens: number; total_tokens: number; prompt_tokens_details?: { cached_tokens: number } }): void {
	usageCorrelationMap.set(correlationId, usage);
}

export function retrieveUsageByCorrelation(correlationId: string): { prompt_tokens: number; completion_tokens: number; total_tokens: number; prompt_tokens_details?: { cached_tokens: number } } | undefined {
	const usage = usageCorrelationMap.get(correlationId);
	if (usage) {
		usageCorrelationMap.delete(correlationId);
	}
	return usage;
}
