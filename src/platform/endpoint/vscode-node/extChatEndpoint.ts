...
import { retrieveCapturingTokenByCorrelation, retrieveUsageByCorrelation, storeCapturingTokenForCorrelation } from '../../requestLogger/node/requestLogger';
...
export class ExtensionContributedChatEndpoint implements IChatEndpoint {
	...
	async makeChatRequest2({
		...
		try {
			...
			if (text || numToolsCalled > 0 || hasThinking) {
				// Retrieve real usage data stored by the BYOK provider via side channel
				const realUsage = retrieveUsageByCorrelation(ourRequestId);
				return {
					type: ChatFetchResponseType.Success,
					requestId,
					serverRequestId: requestId,
					usage: realUsage ?? { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0, prompt_tokens_details: { cached_tokens: 0 } },
					value: text,
