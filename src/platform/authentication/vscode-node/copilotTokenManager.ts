...
export class VSCodeCopilotTokenManager extends BaseCopilotTokenManager {
	...
	private async _auth(): Promise<TokenInfoOrError> {
		...
		if (session) {
			...
			// Always try anonymous access via devDeviceId, even without allowAnonymousAccess.
			// This enables BYOK-only usage (DeepSeek, Mimo, etc.) without a GitHub account.
			this._logService.info(`No GitHub session found, attempting anonymous access with devDeviceId`);
			const tokenResult = await this.authFromDevDeviceId(env.devDeviceId);
			if (tokenResult.kind === 'success') {
				this._logService.info(`Got Copilot token for devDeviceId`);
				this._logService.info(`Copilot Chat: ${this._envService.getVersion()}, VS Code: ${this._envService.vscodeVersion}`);
			} else {
				this._logService.warn('Anonymous auth also failed, proceeding without Copilot token (BYOK-only mode)');
				// Return a synthetic no-auth result instead of failure.
				// This allows BYOK models to work even when Copilot API is unreachable.
				return {
					kind: 'success',
					token: '',
					expires_at: Date.now() / 1000 + 999999999,
					refresh_in: 999999999,
