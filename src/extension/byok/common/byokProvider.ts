...
export function resolveModelInfo(modelId: string, providerName: string, knownModels: BYOKKnownModels | undefined, modelCapabilities?: BYOKModelCapabilities): IChatModelInformation {
	...
	if (knownModelInfo?.includeReasoning !== undefined) {
		modelInfo.includeReasoning = knownModelInfo.includeReasoning;
	}
	if (knownModelInfo?.temperature !== undefined) {
		modelInfo.temperature = knownModelInfo.temperature;
	}
	if (knownModelInfo?.top_p !== undefined) {
		modelInfo.top_p = knownModelInfo.top_p;
	}
	if (knownModelInfo?.top_k !== undefined) {
		modelInfo.top_k = knownModelInfo.top_k;
	}
	return modelInfo;
}
...
export function byokKnownModelToAPIInfo(providerName: string, id: string, capabilities: BYOKModelCapabilities): LanguageModelChatInformation {
	...
	// Add thinking effort selector for thinking-capable models with supportsReasoningEffort
	if (capabilities.thinking && capabilities.supportsReasoningEffort?.length) {
