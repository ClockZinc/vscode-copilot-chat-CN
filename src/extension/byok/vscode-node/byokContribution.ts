...
export class BYOKContrib extends Disposable implements IExtensionContribution {
	...
	private async _authChange(authService: IAuthenticationService, instantiationService: IInstantiationService) {
		// Register BYOK providers even without a Copilot token.
		// This allows BYOK-only usage (e.g., DeepSeek, Mimo) without a GitHub Copilot subscription.
		const hasToken = !!authService.copilotToken;
		const byokAllowed = hasToken ? isBYOKEnabled(authService.copilotToken!, this._capiClientService) : true;
		if (byokAllowed && !this._byokProvidersRegistered) {
