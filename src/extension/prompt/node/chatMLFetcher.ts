...
export class ChatMLFetcherImpl extends AbstractChatMLFetcher {
	...
	public async fetchMany(opts: IFetchMLOptions, token: CancellationToken): Promise<ChatResponses> {
		...
		try {
			...
			if (!payloadValidationResult.isValid) {
				...
				// For BYOK models, skip Copilot token requirement — they use their own API keys
				let copilotToken: CopilotToken | undefined;
				const isBYOK = isBYOKModel(chatEndpoint) >= 0;
				try {
					copilotToken = await this._authenticationService.getCopilotToken();
				} catch (e) {
					if (!isBYOK) {
						throw e; // Re-throw for non-BYOK models
					}
					this._logService.info('[ChatMLFetcher] No Copilot token available, proceeding with BYOK model');
				}
	...
	private async _doFetchViaWebSocket(
		...
		// Skip for BYOK models — they use the user's own API key and don't have CAPI quota
