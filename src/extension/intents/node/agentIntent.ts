...
export class AgentIntentInvocation extends EditCodeIntentInvocation implements IIntentInvocation {
	...
	override async buildPrompt(
		...
		// For BYOK (non-copilot vendor) models, always enable background compaction since they don't
		// have server-side context management and need local summarization to handle long conversations.
		const isBYOK = this.endpoint.modelProvider !== 'copilot';
		const backgroundCompactionEnabled = summarizationEnabled && !inlineSummarizationEnabled && (isBYOK || this.configurationService.getExperimentBasedConfig(ConfigKey.BackgroundCompaction, this.expService));
