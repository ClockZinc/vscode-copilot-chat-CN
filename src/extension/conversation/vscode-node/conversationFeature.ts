...
export class ConversationFeature implements IExtensionContribution {
	...
	constructor(
		...
		if (authenticationService.copilotToken) {
			...
			this.logService.info(`ConversationFeature: No Copilot token, activating for BYOK-only mode`);
			// Activate even without a token so BYOK models can work
			this.activated = true;
			activationBlockerDeferred.complete();
		}
		...
		this._disposables.add(authenticationService.onDidAuthenticationChange(async () => {
			...
			// Always activate conversation feature, even without a Copilot token.
			// This allows BYOK-only usage (e.g., DeepSeek, Mimo) without a GitHub Copilot subscription.
			this.activated = true;
	...
	private registerCopilotTokenListener() {
		this._disposables.add(this.authenticationService.onDidAuthenticationChange(() => {
			const hasToken = this.authenticationService.copilotToken !== undefined;
			this.logService.info(`copilot token sku: ${this.authenticationService.copilotToken?.sku ?? 'none (BYOK-only mode)'}`);
			// Keep chat enabled even without a Copilot token for BYOK-only usage
			this.enabled = true;
		}));
	}
