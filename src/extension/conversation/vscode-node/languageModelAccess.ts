...
import { retrieveCapturingTokenByCorrelation, runWithCapturingToken, storeUsageForCorrelation } from '../../../platform/requestLogger/node/requestLogger';
...
function buildConfigurationSchema(endpoint: IChatEndpoint): { configurationSchema?: vscode.LanguageModelConfigurationSchema } {
	...
	// Only enable effort picker for Claude, GPT, and BYOK thinking-capable models
	const family = endpoint.family.toLowerCase();
	const isBYOK = endpoint.modelProvider !== 'copilot';
	if (!isBYOK && !family.startsWith('claude') && !family.startsWith('gpt-')) {
		return {};
	}

	const preferred = (family.startsWith('claude') || family.startsWith('deepseek')) ? 'high' : 'medium';
...
export class LanguageModelAccess extends Disposable implements IExtensionContribution {
	...
	private async _getToken(): Promise<CopilotToken | undefined> {
		try {
			...
			// Synthetic no-auth token (empty token string) means BYOK-only mode.
			// Don't return it — this prevents Copilot model registration.
			if (copilotToken.isNoAuthUser && !copilotToken.token) {
				return undefined;
			}
			return copilotToken;
		} catch (e) {
...
export class CopilotLanguageModelWrapper extends Disposable {
	...
	private async _provideLanguageModelResponse(_endpoint: IChatEndpoint, _messages: Array<vscode.LanguageModelChatMessage | vscode.LanguageModelChatMessage2>, _options: vscode.ProvideLanguageModelChatResponseOptions, extensionId: string | undefined, callback: FinishedCallback, token: vscode.CancellationToken): Promise<void> {
		...
		*/
		...
		// Store real usage data for retrieval by ExtensionContributedChatEndpoint
		// across the VS Code IPC boundary (which normally discards usage).
		if (result.type === ChatFetchResponseType.Success && correlationId && result.usage) {
			storeUsageForCorrelation(correlationId, result.usage);
		}
		...
		if (result.type !== ChatFetchResponseType.Success) {
			if (result.type === ChatFetchResponseType.ExtensionBlocked) {
				...
				try { copilotPlan = (await this._authenticationService.getCopilotToken()).copilotPlan; } catch { /* BYOK-only mode */ }
