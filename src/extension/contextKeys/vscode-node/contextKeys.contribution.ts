...
export class ContextKeysContribution extends Disposable {
	...
	private async _inspectContext() {
		...
		try {
			...
			// For BYOK-only mode: if GitHub login fails, treat as activated
			// so the chat panel works with user-configured BYOK models (DeepSeek, Mimo, etc.)
			if (e instanceof GitHubLoginFailedError) {
				this._logService.info(`[context keys] GitHub login failed, activating in BYOK-only mode`);
				key = welcomeViewContextKeys.Activated;
				error = undefined;
			}
		}
